...
public class ImageSelectionProfilePopupDialog extends Activity implements IAddImageInterface{//com.i_just_call_to_say.activities.greetings.ImageSelectionProfilePopupDialog

	private static final int PICTURE_GALLERY_REQUEST = 2572;
	private static final int CAMERA_PIC_REQUEST = 1337;
	public String picturePath = "";
	private RelativeLayout rel_gallery,rel_camera,rel_remove;
...
//		getWindow().setFlags(WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL,WindowManager.LayoutParams.FLAG_NOT_TOUCH_MODAL);
	...
	private void initView(){
		...
		rel_remove = (RelativeLayout)findViewById(R.id.rel_remove);
		...
		rel_remove.setBackgroundResource(R.drawable.btn_removepic_01_dark);
		rel_remove.setTag(R.drawable.btn_removepic_01_dark);
	}
	...
	private void switchBackground(){
		...
		if(Integer.parseInt(""+rel_remove.getTag()) == R.drawable.btn_removepic_01_dark){
			rel_remove.setBackgroundResource(R.drawable.btn_removepic_01);
			rel_remove.setTag(R.drawable.btn_removepic_01);
		}else{
			rel_remove.setBackgroundResource(R.drawable.btn_removepic_01_dark);
			rel_remove.setTag(R.drawable.btn_removepic_01_dark);
		}
	...
	private String saveScaledBitMap(Bitmap bitmap, String extension) {
		...
		try {
			...
			// if(!file.exists())
			file.createNewFile();
           // Toast.makeText(ImageSelectionProfilePopupDialog.this, "bitmap "+bitmap, 200);
			...
			//Toast.makeText(ImageSelectionProfilePopupDialog.this, "Exception  "+e.getMessage(), 200);
			e.printStackTrace();
		} finally {
			try {
				out.close();
	...
	private void sendImagePath( String filePath , final boolean isVideo){
		...
		if(fileSize < 1024 * 2){
			...
			DialogUtility.showMessageOkWithCallback("You cannot upload images/videos of size larger then 2 MB.", this, new AlertDialogCallBack() {
		...
		if(resultCode == RESULT_OK){
			...
			case CAMERA_PIC_REQUEST:
				if (data != null) {
					...
					picturePath = saveScaledBitMap(BitmapFactory.decodeFile(picturePath),picturePath.substring(picturePath.indexOf("."), picturePath.length()));
						...
						if (Build.VERSION.SDK_INT <19){
							...
							if(picturePath != null){
								...
								if(picturePath != null){
									...
									Toast.makeText(ImageSelectionProfilePopupDialog.this, "No image is selected", 2000).show();
								}
							}else{
								GoogleImageProcessUtility googleImageProcessUtility = new GoogleImageProcessUtility(ImageSelectionProfilePopupDialog.this,selectedImage.toString(),new OnProcessCompleteListener() {
									@Override
